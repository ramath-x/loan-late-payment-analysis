...
Stored Procedure `get_arrears_history` ใช้ Common Table Expressions (CTEs) และฟังก์ชันวินโดว์เพื่อคำนวณประวัติการค้างชำระเงิน
...
2. **CTE `late_payments`**:
    - กรองแถวที่ค่า `prev_arrears` ไม่เป็น NULL เพราะเป็นแรกของ loan_id 
...
3. **CTE `delinquency_periods`**:
    - กรองช่วงเวลาที่ค้างชำระอย่างต่อเนื่องออก เพื่อเอาเวลาของแถวบนกับแถวล่างมาคำนวณ ดูได้จากรูป loan_id 1005 
...
4. **CTE `arrears_history`**:
    ...
    - เพิ่มคอลัมน์ `date_diff` เพื่อคำนวณระยะเวลาของแต่ละช่วงค้างชำระ ใช้ฟังก์ชัน `IFNULL` เผื่อกรณีที่มียอดค้างชำระจนถึงปัจจุบันแล้วยังไม่จ่าย
    - เพิ่มคอลัมน์ `start_date` และ `end_date` เพื่อคำนวณระยะเวลาของแต่ละช่วงค้างชำระ 
	- ใน `end_date` ใช้ฟังก์ชัน `IFNULL` เผื่อกรณีที่มียอดค้างชำระจนถึงปัจจุบันแล้วยังไม่จ่าย ดูจากรูปตัวอย่าง
...
5. **การคำนวณสรุปข้อมูล**:
	...
	- กรองแถวที่ค่า `arrears_amount` ที่มากกว่า 0 และ `prev_arrears` ที่น้อยกว่าหรือเท่ากัย 0 เพื่อคำนวณวันที่มีการชำระ
